...
static void divide_vetor(int N, int P, int *counts, int *displs) {
    // Divide N elementos em P blocos (o mais equilibrado possível)
    int base = N / P;
    int resto = N % P;
    int desloc = 0;
    for (int i = 0; i < P; i++) {
...
int main(int argc, char *argv[]) {
    ...
    int N = 10; // máximo de 10 elementos conforme enunciado
    int vet[10];
    ...
    if (rank == 0) {
        ...
        // Escravos recebem tamanho e pedaço
    ...
    // ===== Cada processo calcula soma e multiplicação locais =====
    int local_sum = 0;
    int local_mul = 1;
    ...
    // ===== Envio dos resultados (não bloqueante) =====
    ...
    if (rank == 0) {
        ...
        // Posta Irecv para todos (inclui o próprio 0 para uniformizar)
        for (int r = 0; r < nprocs; r++)
            MPI_Irecv(all_results[r], 2, MPI_INT, r, 200, MPI_COMM_WORLD, &reqs[r]);

        // Envia também seu resultado (autoenvio) — mantém o padrão mestre/escravo
        ...
        // Escravos enviam resultados com Isend
        MPI_Request req;
